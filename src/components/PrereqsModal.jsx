      ...
      <div className="glass-panel w-full max-w-lg rounded-2xl border border-slate-700/80 p-6 shadow-2xl relative max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between pb-4 border-b border-slate-800">
          <div>
            <h3 id={MODAL_TITLE_ID} className="text-base font-bold text-white flex items-center gap-2">
              <GitFork className="w-5 h-5 text-brand-400" aria-hidden="true" />
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-2">
          ...
          {reqs.length === 0 ? (
            ...
            <ul className="space-y-2" aria-label="Lista de requisitos previos">
              {reqs.map(p => {
                ...
                return (
                  <li key={p.id} className="flex items-center justify-between p-2.5 bg-slate-900/90 border border-slate-800 rounded-xl">
                    ...
                    <button
                      ...
                      aria-label={`Eliminar requisito: ${p.materia_requerida?.nombre}`}
        ...
        <div className="pt-4 border-t border-slate-800/90 bg-slate-900/50 rounded-xl p-3 mt-2">
          <h4 className="text-xs font-semibold text-slate-300 mb-2.5 flex items-center gap-1.5">
            <PlusCircle className="w-3.5 h-3.5 text-brand-400" aria-hidden="true" />
            Agregar Requisito Previo
          </h4>
          <div className="space-y-3">
            <div>
              <label htmlFor="prereq-materia" className="block text-[11px] font-medium text-slate-400 mb-1">Materia Predecesora</label>
              <select id="prereq-materia" value={materiaReqId} onChange={e => setMateriaReqId(e.target.value)}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label htmlFor="prereq-tipo" className="block text-[11px] font-medium text-slate-400 mb-1">Condición Exigida</label>
                <select id="prereq-tipo" value={tipo} onChange={e => setTipo(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-700/80 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-brand-500">
              ...
              <div className="flex items-end">
                <button
                  type="button"
                  onClick={handleAdd}
                  aria-label="Vincular nueva correlatividad"
                  className="w-full py-2 bg-brand-600 hover:bg-brand-500 text-white rounded-xl text-xs font-semibold transition-colors flex items-center justify-center gap-1.5"
                >
